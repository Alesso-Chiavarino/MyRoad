...
const LoginForm = ({ handleChange, handleSubmit, inputType, showPassword }) => {
    ...
    return (
        <div className='bg-white/50 p-[1px] rounded-md'>
            <div className="bg-[url('/img/auth-wave.svg')] w-fit flex justify-center flex-col min-w-[600px] p-10 rounded-md">
                <h1 className='text-4xl font-extrabold text-white'>User Login</h1>
                <form action="" onSubmit={handleSubmit} className='flex flex-col gap-10 pt-10 w-full'>
                    ...
                    <button className='bg-white rounded-md py-2 font-bold hover:bg-white/90'>Login</button>
                    <div className='flex gap-1 text-white'> <span>Don't have an account?</span><Link href={'/auth/register'} className='text-[#7148FC]'>Register here.</Link> </div>
                </form>
            </div>
        </div>
    )
